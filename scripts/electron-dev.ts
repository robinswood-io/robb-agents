...
// Detects instance number from folder name suffix (e.g., robinswood-agents-1 → instance 1)
function detectInstance(): void {
  ...
  if (match) {
    ...
    process.env.CRAFT_APP_NAME = `Robinswood Agents [${instanceNum}]`;
    process.env.CRAFT_CONFIG_DIR = join(process.env.HOME || "", `.robinswood-agents-${instanceNum}`);
...
function getElectronEnv(): Record<string, string> {
  ...
  return {
    ...
    CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || "Robinswood Agents",
