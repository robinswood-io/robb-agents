...
- **Cross-platform release and update contract** — macOS, Windows and Linux now share one fail-closed publication validator for artifacts, updater manifests, provenance, signatures, SBOM and canonical checksums. Public release helpers install or update the correct GitHub Release artifact only after size/hash checks and platform signature verification, while CI exercises the contract on all three operating systems.
...
- **Visible autonomous resolution** — Sessions now retain a compact, inspectable timeline when an agent diagnoses a tool failure, switches to a safe browser fallback, or encounters a classified human-only blocker such as MFA, credentials, or unavailable access. This avoids opaque “please help” dead ends while preserving safeguards for external and irreversible actions.
