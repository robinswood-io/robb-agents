...
export function buildGeneratorPrompt(goal: string, title?: string): string {
  return [
    ...
    '- Give every node one clear specialist responsibility (for example research, implementation, testing, security review, or synthesis). Do not create several generic nodes with overlapping work.',
    '- Each node prompt must tell the specialist to work autonomously through completion, inspect errors, change approach after a failure, and execute concrete verification before reporting success.',
    ...
    '      prompt: the full specialist instruction, deliverable, and verification for this subtask (may include ${nodes.<id>.output})',
    ...
    '      prompt: Act as a codebase research specialist. List every auth call site and how it is used. Work autonomously, cross-check the list with repository search, and report the exact files inspected.',
    '    - id: design',
    '      title: Design new auth',
    '      prompt: "Act as an authentication architecture specialist. Design the new session-based auth using the audit: ${nodes.audit.output}. Check the design against every audited call site before reporting completion."',
    '      depends_on: [audit]',
    '',
